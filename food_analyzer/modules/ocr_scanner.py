...
def get_ocr_reader():
    ...
    if EASYOCR_AVAILABLE:
        try:
            # EasyOCR поддържа множество езици - включваме български и английски
...
def extract_text_from_image(image: Image.Image, reader=None, engine: str = None) -> dict:
    ...
    Args:
        ...
        reader: Инициализиран OCR четец
        engine: Тип на двигателя ('easyocr', 'tesseract')
    
    Returns:
        dict с извлечен текст и метаданни
    """
    result = {
        "raw_text": "",
        "confidence": 0.0,
    ...
    if reader is None:
        # Ако няма OCR, връщаме примерен текст за демонстрация
        result["raw_text"] = _get_demo_ocr_text()
        result["success"] = True
        result["engine"] = "demo"
    ...
    try:
        if engine == "easyocr":
            # EasyOCR - по-точен за многоезичен текст
            import numpy as np
            ...
            for (bbox, text, conf) in ocr_results:
                if conf > 0.3:  # Филтрираме ниско достоверни резултати
                    texts.append(text)
            ...
            # pytesseract - по-бърз, но по-малко точен
            config = '--oem 3 --psm 6 -l bul+eng'
            text = reader.image_to_string(image, config=config)
            data = reader.image_to_data(image, config=config, output_type=reader.Output.DICT)
            
            # Изчисляваме средна достоверност
        ...
        result["error"] = str(e)
        # Fallback към демо данни
        result["raw_text"] = _get_demo_ocr_text()
        result["success"] = True
        result["engine"] = "demo_fallback"
